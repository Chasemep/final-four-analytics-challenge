...
pred_df_train <- train_copy[c(-28,-39)]
...
for (s in unique(pred_df_combined$Season)) {
  # Hide current qualify to avoid interference
  season_mask <- pred_df_combined$Season == s
  
  # For predictions, we use the probability from model_qualify
  X_season <- as.matrix(pred_df_combined[season_mask, base_features])
  # Impute if needed (already done for test, but good for combined)
  ...
  # Now rank the 68 qualified teams by ensemble seed
